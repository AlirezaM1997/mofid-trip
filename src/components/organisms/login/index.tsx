...
const Login = ({ protectedScreen }) => {
  const {tr} = useTranslation()
  const dispatch = useDispatch()
  const navigation = useNavigation()
  const isAuthenticated = useIsAuthenticated()

  const handleLogin = (address: string) => navigation.navigate(address)
  ...
  return (
    <View style={style.container}>
      ...
      <View style={style.buttonContainer}>
        <Button size="lg" color="secondary" containerStyle={style.buttonItem} onPress={() => handleLogin("LoginScreen")}>
          {tr('Login')}
        </Button>
        <Button size="lg" color="secondary" type="outline" containerStyle={[style.buttonItem]} onPress={() => handleLogin("RegisterScreen")}>
          {tr('Register')}
        </Button>
      </View>
    </View>
