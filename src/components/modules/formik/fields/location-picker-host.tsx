...
export type LocationPickerHostProps = FieldProps & ViewProps;
...
const LocationPickerHost = ({ field, form, ...props }: LocationPickerHostProps) => {
  ...
  const handlePress = () => {
    ...
    setFieldTouched("accommodation.lng", true);
    handleOpen();
  ...
  const handleSubmit = () => {
    setFieldValue("accommodation.lat", lat);
    setFieldValue("accommodation.lng", lng);
    handleClose();
  };
  ...
  return (
    <>
      <Pressable style={styles.container(theme)} onPress={handlePress}>
        {form.values.accommodation.lat && form.values.accommodation.lng ? (
          <>
            <Image
              containerStyle={{
                width: 52,
